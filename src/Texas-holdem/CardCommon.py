...
class PokerHelper:
    ROYAL_STRAIGHT_FLUSH = ['Royal-Straight-Flush!!!', 10000]  # 스트레이트 플러쉬 중 가장 높은 숫자 5개로 조합된 카드. (확률 0.0032%)
    BACK_STRAIGHT_FLUSH = ['Back-Straight-Flush', 5000]
    STRAIGHT_FLUSH = ['Straight-Flush', 3000]  # 같은 무늬에 연결되는 숫자 5개로 조합된 카드. (확률 0.029%)
    FOUR_CARDS = ['Four-Cards', 2000]  # 같은 숫자 4장(JJJJ, QQQQ, 5555 등)으로 조합된 카드. (확률 0.16%)
    FULL_HOUSE = ['Full-House', 1000]  # 트리플과 페어로 조합된 카드. (확률 2.63%)
    FLUSH = ['Flush', 800]  # 같은 무늬 5장으로 구성된 카드. (확률 3.03%)

    MOUNTAIN = ['Mountain', 500]
    BACK_STRAIGHT = ['Back-Straight', 450]
    STRAIGHT = ['Straight', 400]  # 5장의 숫자가 연속으로 구성된 카드. (확률 4.55%)

    TRIPLE = ['Triple', 300]  # 세 장의 같은 숫자로 조합된 카드. (확률 4.8%)
    TWO_PAIR = ['Two-Pair', 200]  # 두 개의 원페어로 구성된 카드 (확률 23.5%)
    SINGLE_PAIR = ['Single-Pair', 100]  # 원페어. 두 장의 같은 숫자로 구성된 카드. (확률 43.8%)

    HIGH_CARD = ['High-Card', 10]  # 보드와 무관한 카드로, 높은 숫자를 가진 사람이 이긴다.
    ...
    # 키커를 이용하여 Rank 비교
    # 키커란 2명 이상의 플레이어가 동일한 족보를 들었을 때, 그 족보를 제외한 나머지 카드에서의 높은 카드를 말합니다.
    @staticmethod
            ...
            if h1_second_pair_rank > h2_second_pair_rank:
                ...
                if len(r1.kicker) != len(r2.kicker):
                    ...
                    if r1_kicker[0].rank > r2_kicker[0].rank:
                        print('r1 kicker rank (' + str(r1_kicker[0].rank) + ') > r2 kicker rank (' + str(
                            r2_kicker[0].rank) + ')')
                        return 1
